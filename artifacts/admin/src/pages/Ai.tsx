...
export default function AiPage() {
  ...
  return (
    <div className="space-y-4 md:space-y-6">
      <div>
        <h1 className="text-xl md:text-2xl font-bold">AI / Proxy API</h1>
      ...
      {status && (
        <div className="bg-card border rounded-xl p-4 md:p-6 space-y-4">
          <div className="flex items-center gap-3">
            ...
            <span className="font-semibold text-base md:text-lg">{status.available ? "✅ Proxy API доступен" : "❌ Proxy API недоступен"}</span>
          </div>
          ...
          <button
            ...
            className="bg-primary text-primary-foreground px-4 py-2 rounded-lg text-sm font-medium hover:opacity-90 transition disabled:opacity-50 w-full sm:w-auto"
          >
      ...
      <div className="bg-card border rounded-xl overflow-hidden">
        <div className="px-3 md:px-4 py-3 border-b bg-muted/50">
        ...
        <div className="divide-y max-h-96 overflow-y-auto">
          ...
          {logs.map((log) => (
            <div key={log.id} className="px-3 md:px-4 py-3 text-xs">
