...
import { classifyUserInput, generateReaction, answerQuestion, isAiAvailable, generateStageReply, type KnownFacts } from "./ai.js";
import {
...
import {
  ...
  LEAD_COMMENT_PROMPT,
  LEAD_DONE_PROMPT,
  OBJECTION_PRICE_PROMPT,
  OBJECTION_PYRAMID_PROMPT,
  OBJECTION_HEALTH_PROMPT,
  OBJECTION_INCOME_PROMPT,
  OBJECTION_SOFT_DECLINE_PROMPT,
  QNA_PROMPT,
  OFF_TOPIC_PROMPT,
  EARLY_REGISTRATION_PROMPT,
} from "./prompts.js";
...
async function handleFinalQuestion(bot: TelegramBot, chatId: number, session: BotSession) {
  ...
  await saveMessage(session.id, "bot", reply || TEXTS.finalQuestion, "final_question");
}
...
export async function handleMessage(bot: TelegramBot, msg: Message) {
  ...
  switch (stage) {
    ...
    case "family_question": {
      ...
      await handleBigCalculation(bot, chatId, { ...session, familyAdults: count });
      break;
    }
    case "lead_capture_name": {
      await saveMessage(session.id, "user", text, stage);
      await db.update(userSessionsTable).set({ currentStage: "lead_capture_contact", displayName: text.trim(), updatedAt: new Date() }).where(eq(userSessionsTable.id, session.id));
    ...
    default: {
      ...
      await bot.sendMessage(chatId, response, { parse_mode: "Markdown", reply_markup: getReplyKeyboard() });
    }
  }
}
...
async function showMainMenu(
  ...
  const nameInTitle = session.displayName || partner?.name || session.firstName || "друг";
  const title = partner
    ? `🏠 *Главное меню*\n\nПривет, *${nameInTitle}*! Ты партнёр Greenleaf.\n\nЭтап: \`${session.currentStage}\``
    : `🏠 *Главное меню*\n\nПривет, *${nameInTitle}*!\n\nЭтап: \`${session.currentStage}\``;
