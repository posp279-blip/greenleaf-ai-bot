...
import { handleJarvisV18Message, handleJarvisV18Callback, initJarvisV18 } from "./jarvisV18.js";
...
async function configureJarvisIdentity(currentBot: TelegramBot): Promise<void> {
  try {
    await currentBot.setMyCommands([
      ...
      { command: "site", description: "Проверить связь с персональным сайтом" },
...
export async function startBot(webhookUrl?: string): Promise<void> {
  ...
  if (!webhookInfo.url || webhookInfo.url !== webhookUrl) {
    ...
    bot.on("message", async (msg) => {
      try { await handleJarvisV18Message(bot!, msg); }
      catch (err) {
        logger.error({ err, chatId: msg.chat.id }, "Error handling Jarvis v18 message");
    ...
    bot.on("callback_query", async (query) => {
      try {
        ...
        if (isAdminCallback(data)) await handleAdminCallback(bot!, query); else await handleJarvisV18Callback(bot!, query);
      } catch (err) { logger.error({ err }, "Error handling callback query"); }
    });
    bot.on("polling_error", (err) => logger.error({ err }, "Telegram polling error"));
    logger.info("Telegram Jarvis v18 started in polling mode");
  } else logger.info("Telegram Jarvis v18 started in webhook mode");
}
...
export async function handleWebhookUpdate(update: Update): Promise<void> {
  ...
  try {
    if (update.message) await handleJarvisV18Message(b, update.message);
    else if (update.callback_query) {
      const data = update.callback_query.data || "";
      if (isAdminCallback(data)) await handleAdminCallback(b, update.callback_query); else await handleJarvisV18Callback(b, update.callback_query);
    }
  } catch (err) { logger.error({ err }, "Error handling Jarvis v18 webhook update"); }
}
